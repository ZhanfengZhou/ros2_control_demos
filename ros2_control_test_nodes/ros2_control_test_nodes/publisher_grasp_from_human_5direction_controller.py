...
class PublisherJointTrajectory(Node):
    def __init__(self):
        ...
        self.declare_parameter("grasp_direction", 1)
        self.declare_parameter("goal_names", ["pos0", "pos1"])
        ...
        self.grasp_direction = self.get_parameter("grasp_direction").value
    ...
    def timer_callback(self):
        ...
        if self.starting_point_ok:
            ...
            if self.i == 0:
                ...
                self.get_logger().info('Publishing traj for original point_{}'.format(self.i))
            else:
                traj = JointTrajectory()
                traj.joint_names = self.joints
                point = JointTrajectoryPoint()
                point.positions = self.joints_goals[self.grasp_direction]  #the inverse kinematics results
                point.time_from_start = Duration(sec=self.trajectory_duration)

                traj.points.append(point)
                self.publisher_.publish(traj)
                self.get_logger().info('Publishing traj for goal point_{}'.format(self.grasp_direction))
...
#            raise Exception('The input goals is incorrect. Facing backward!')
        ...
        if input_goals_ok :
            ...
            joints_limits['wrist_1_joint'] = [math.radians(r) for r in [float(angle) for angle in [-180, 130]] ]     # !!!!!!change 100 to 10
